...
def characteristic_material_strength(SMYS, α_U, material=None, T=None, f_ytemp=None):
    ...
    415000000.0
    >>> char_mat_strength(450.e6, α_U=1.0, temperature=100., material='DSS')
    360000000.0
    """
    #α_U = alpha_U
...
def material_strength_derating(T, material=None, curve=None):
    ...
    if not curve:
        ...
        if re.sub(r"\s|-", "", material).upper() in ["CMN", "13CR"]:
            curve = [(50., 0.), (100., 30.e6), (200., 70.e6) ]
        elif re.sub(r"\s|-", "", material).upper() in ["22CR", "25CR", "DSS"]:
